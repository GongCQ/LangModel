...
# https://www.ctolib.com/docs-Tensorflow-c-Tensorflow5.html
...
class PTBModel(object):
    def __init__(self, is_training, batch_size, num_steps):
        ...
        # [按3：此处是一个word embedding层]
        inputs = tf.nn.embedding_lookup(embedding, self.input_data)

        # 只在训练时使用dropout。
        # [按4：此处在word embedding层上也加dropout]
...
def main(_):
    # 获取原始数据。
    # [按1：这里所读出的train_data是一个非常长的list，它把ptb.train.txt文件中所有的句子都串到一起，
    #  且每个词都用一个整型的ID表示。在此例中，分句符的ID是2，当然并不一定需要是2，你只需要把分句符按普通的词赋予ID就行了
    #  同样，在我们的应用案例中，由于还有其他标点符号，也只需要给各标点赋予一个普通的ID就行了]
